...
def process_data_for_cnn(embeddings, all_words, pos_words, neg_words):
    ...
    label = np.array([1] * pos_data.shape[0] + [0] * neg_data.shape[0])
    return data, label
...
def mv_knn(men_vec, word_list, k, out_file, chunk_size=10000, use_cosine=False):
    ...
    if not os.path.exists(out_file):
        ...
        for tt in range(times):
            ...
            s = e
            logging.info(
                str((tt + 1) * chunk_size / men_vec.shape[0]) + " % vectors processed."
            )
    ...
    if is_train:
        if labels is not None:
            sampler = MPerClassSampler(
                labels, m=1, batch_size=batch_size, length_before_new_iter=len(labels)
            )
...
def preprocess_transformed_vectors_for_cnn(
    transformed_vectors, word2vector, vec_num_per_word=500
):
    ...
    for i in range(len(uniq_word)):
        ...
        if where_wi.sum() == vec_num_per_word:
            ...
            embeddings[i] = torch.cat(
                (
                    transformed_vectors[where_wi],
                    torch.zeros((vec_num_per_word - where_wi.sum(), dim)),
                ),
                dim=0,
            )
    return embeddings
...
def preprocess_transformed_vectors_for_svm(transformed_vectors, word2vector):
    ...
    for wi in uniq_word:
        where_wi = np.array(word2vector) == wi
        embeddings[wi] = (
            torch.mean(transformed_vectors[where_wi], dim=0).detach().numpy()
        )
    return embeddings
    ...
    if filename.endswith(".txt"):
        with open(filename, "r", encoding="utf-8") as f:
            for line in f:
                ...
                embeddings[items[0].lower()] = [float(i) for i in items[1:]]
        embeddings["missing_word"] = np.mean(
            np.array([i for i in embeddings.values()]), axis=0
        )
...
def pi_metric(arr_a, arr_b):
    ...
    if len(set_a & set_b) > 0:  # if intersection of a and b is not empty
        ...
        cnt_a = [
            list(arr_a).count(i) if i in arr_a else 0 for i in ab_union
        ]  # count for each word in array a
        cnt_b = [
            list(arr_b).count(i) if i in arr_b else 0 for i in ab_union
        ]  # count for each word in array a
...
def build_positive_pair(
    ...
    method=1,
    k=5,
    mv_knn_file="mv_knn.npy",
    use_cosine=False,
    word_knn_file="word_knn.json.bz2",
    olp_th=0.5,
    out_path="pos_pair",
):
    # Input
    ...
    if method == 1:
        ...
        out_file = os.path.join(
            out_path, str(method) + "_" + str(k) + "_" + str(olp_th) + ".npy"
        )
    ...
    if os.path.exists(out_file):
        ...
        if (
            method == 1
        ):  # if two in the mention vector space are similar, then they are similar
            knn_idx = mv_knn(men_vec, word_list, k, mv_knn_file, use_cosine=use_cosine)[
                0
            ]
            # if two mv are neighbors, then they are similar
            mv_idx_pairs = list(
                zip(np.repeat(range(len(knn_idx)), k), knn_idx.reshape(-1))
            )

        elif (
            method == 2
        ):  # if two mv's neighbors of word are similar, then they are similar
            word_knn_tmp = np.array(load_mv_top_words(word_knn_file)["top_words"])[
                :, :k
            ]
            word_knn = []
            ...
            cnt = 0
            all_pairs = list(combinations(range(len(word_knn)), 2))
            total = len(all_pairs)
            for i, j in all_pairs:
                if cnt % 10000 == 0:
                    logging.info(str(round(cnt / total, 4) * 100) + " % processed")
...
def build_positive_triple(properties, sent_path, out_path):
    ...
    if not os.path.exists(file_pp):
        ...
        for prop_i in range(len(properties)):
            ...
            prop_sents = load_text(
                prop_file
            )  # sentences mentioned a same property, can be positive pair
            ...
            prop_prompt_idx.extend(
                list(
                    zip(
                        [prop_i] * len(prop_sent_pairs),
                        range(prompt_pair_idx, prompt_pair_idx + len(prop_sent_pairs)),
                    )
                )
            )
...
def init_word_embedding_for_cnn(
    embeddings, all_words, target_words, remain_word_id=None
):
...
def process_filtered_data_for_cnn(
    embeddings, all_words, pos_words, neg_words, remain_word_id=None
):
    pos_data = init_word_embedding_for_cnn(
        embeddings, all_words, pos_words, remain_word_id
    )
    neg_data = init_word_embedding_for_cnn(
        embeddings, all_words, neg_words, remain_word_id
    )
...
def preprocess_filter_transformed_vectors_for_svm(
    transformed_vectors, word2vector, remain_word_id
):
    uniq_word = uniq_word_ordered(word2vector)
    dim = transformed_vectors.shape[1]
    embeddings = dict()
    ...
    for w in uniq_word:
        ...
        if w in remain_word_id:
            embeddings[w] = torch.mean(
                transformed_vectors[where_w][remain_word_id[w]], dim=0
            )
        else:
...
def write_csv(file, properties, results):
    ...
    with open(file, "w") as f:
        ...
        writer.writerow(
            ["mean"] + np.round(np.mean(np.array(results), axis=0), decimals=4).tolist()
        )
...
def write_txt(file, results, header):
    ...
    output += (
        "\t".join(
            [str(m) for m in np.round(np.mean(np.array(results), axis=0), decimals=4)]
        )
        + "\n"
    )
    output += "\n\n"
