...
def prepare_data_and_models(config):
    ...
    concept_sentence_file = dataset_params["concept_sentence_file"]

    num_workers = 4

    con_sent_dataset = DatasetConceptSentence(concept_sentence_file, dataset_params)
    ...
    log.info(f"concept_sentence_df.shape : {con_sent_dataset.data_df.shape}")
...
def run_model(config, param_dict):
    ...
    for step, batch in enumerate(tqdm(con_sent_dataloader, desc="Iteration")):
