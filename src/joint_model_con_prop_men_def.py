...
class DatasetConceptSentence(Dataset):
    def __init__(self, con_prop_men_def_file, dataset_params):
        if isinstance(con_prop_men_def_file, pd.DataFrame):
            ...
            log.info(
                f"Supplied Concept Mention Definition File is a Dataframe : {self.data_df.shape}",
            )
            ...
            log.info(
                f"Supplied Concept Property File is a Path : {con_prop_men_def_file}"
            )
            ...
            self.data_df = pd.read_csv(
                ...
                sep="\t",
                header=None,
                names=["concept", "property", "mention", "definition"],
                dtype={
                    "concept": str,
                    "property": str,
                    "mention": str,
    ...
    def __getitem__(self, idx):
        ...
        mention = self.data_df["mention"][idx]
        definition = self.data_df["definition"][idx]
    ...
    def get_ids(self, batch):
        ...
        property_prompt = property + " means " + self.mask_token
